...
export function BackgroundImageUpload() {
  ...
  const { backgroundImage, backgroundOpacity, setBackgroundImage, setBackgroundOpacity } =
    useEditorStore();
  ...
  return (
    <div className="p-3 border-t border-ha-border space-y-2">
      <p className="text-ha-muted text-xs uppercase tracking-wider font-semibold">
        Background Image
      </p>
      ...
      >
        {backgroundImage ? (
          ...
          <span className="text-ha-muted text-xs">
            Drop PNG/JPG/WebP or click to upload
          </span>
        )}
      </div>
      <input
      ...
      {/* Controls — only shown when an image is loaded */}
      {backgroundImage && (
        <>
          <div className="flex items-center gap-2">
            <span className="text-ha-muted text-xs w-14 shrink-0">
              Opacity
            </span>
            <input
              type="range"
              ...
              className="flex-1 accent-ha-accent"
            />
            <span className="text-ha-muted text-xs w-8 text-right">
