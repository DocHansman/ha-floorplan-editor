...
export function useWallTool(stageRef: React.RefObject<Konva.Stage | null>) {
  ...
  const handleClick = useCallback(
    (e: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      if (!draft) {
        setDraft({ points: [snapped.x, snapped.y], cursorX: snapped.x, cursorY: snapped.y });
        return;
      }

      setDraft((d) => d && {
        points: [...d.points, snapped.x, snapped.y],
        cursorX: snapped.x,
        cursorY: snapped.y,
      });
    },
  ...
  const handleDblClick = useCallback(
    (e: Konva.KonvaEventObject<MouseEvent>) => {
      ...
      // Need at least 2 points (4 coords) to make a wall segment
