...
const DOMAIN_COLORS: Record<string, string> = {
  ...
  sensor: 'text-gray-400',
  binary_sensor: 'text-purple-400',
  climate: 'text-red-400',
  fan: 'text-blue-400',
  cover: 'text-amber-700',
  lock: 'text-green-500',
  media_player: 'text-indigo-400',
  camera: 'text-gray-500',
  person: 'text-cyan-400',
  automation: 'text-pink-400',
  script: 'text-teal-400',
};
...
function getDomainColor(domain: string) {
  return DOMAIN_COLORS[domain] ?? 'text-ha-muted';
}
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-ha-border">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-ha-muted text-xs bg-ha-border px-2 py-0.5 rounded-full">
            {totalCount}
          </span>
        </div>
        <input
      ...
      <div className="flex-1 overflow-y-auto">
        {grouped.size === 0 && (
          <p className="p-4 text-ha-muted text-sm">No entities found.</p>
        )}
          ...
          return (
            <div key={domain} className="border-b border-ha-border">
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-2 hover:bg-ha-surface text-left"
              >
                <span className={`text-xs font-semibold uppercase tracking-wider ${getDomainColor(domain)}`}>
                  {domain}
                </span>
              ...
              {isExpanded && (
                <ul>
                  {items
                    ...
                    .map(({ entityId, state, name }) => (
                      <li
                        key={entityId}
                        className="px-4 py-2 flex items-center justify-between hover:bg-ha-surface/50 group"
                      >
                        <div className="min-w-0">
                          <p className="text-ha-text text-xs truncate">{name}</p>
                          <p className="text-ha-muted text-xs truncate font-mono">{entityId}</p>
                        </div>
                        <span className="text-ha-muted text-xs ml-2 shrink-0 bg-ha-border px-1.5 py-0.5 rounded">
                          {state}
                        </span>
                      </li>
                    ))}
