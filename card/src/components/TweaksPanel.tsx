...
interface Props {
  ...
  onChange: (overrides: CardOverrides) => void;
}
...
export function loadOverrides(projectUrl: string): CardOverrides {
  try {
    const raw = localStorage.getItem(STORAGE_PREFIX + projectUrl);
...
export function TweaksPanel({ projectUrl, overrides, onChange }: Props) {
  ...
  function update(patch: Partial<CardOverrides>) {
    ...
    onChange(next);
    try {
      localStorage.setItem(STORAGE_PREFIX + projectUrl, JSON.stringify(next));
  ...
  return (
    <div ref={panelRef} style={{ position: 'absolute', bottom: 12, right: 12, zIndex: 10 }}>
      ...
      {open && (
        ...
        >
          <p style={{ fontWeight: 700, marginBottom: 10, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.06em', fontSize: 10 }}>
            Display settings
          </p>
          ...
          <div style={{ marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              {ACCENT_PRESETS.map((c) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    background: c,
                    border: overrides.accentColor === c ? '2px solid #fff' : '2px solid transparent',
                    cursor: 'pointer',
                    flexShrink: 0,
                  }}
                />
              ...
              <label style={{ width: 22, height: 22, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.2)', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <input
                  type="color"
                  value={overrides.accentColor ?? '#3b82f6'}
          ...
          <ToggleRow
            label="Room labels"
            value={overrides.showLabels ?? true}
            onChange={(v) => update({ showLabels: v })}
          />

          {/* Dim inactive */}
          <ToggleRow
            label="Dim inactive rooms"
            value={overrides.dimInactive ?? false}
            onChange={(v) => update({ dimInactive: v })}
          />
        </div>
      )}
    </div>
