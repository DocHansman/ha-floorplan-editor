...
interface Props {
  states: HassStates;
  summary: LovelaceCardConfig['summary'];
  accentColor: string;
}
...
export function SummaryBar({ states, summary, accentColor }: Props) {
  ...
  const lightsOn = lightIds.filter((id) => {
    const e = states[id];
    return e ? isEntityActive(e) : false;
  }).length;

  const windowsOpen = windowIds.filter((id) => {
    const e = states[id];
    return e ? isEntityActive(e) : false;
  }).length;

  const tempEntity = summary.temperature ? states[summary.temperature] : undefined;
  const tempValue = tempEntity
    ? `Ø ${tempEntity.state}${(tempEntity.attributes['unit_of_measurement'] as string | undefined) ?? '°C'}`
    : null;
  ...
  return (
    ...
    >
      {parts.map((p) => (
        <span key={p} style={{ color: '#e5e7eb' }}>{p}</span>
      ))}
    </div>
  );
}
